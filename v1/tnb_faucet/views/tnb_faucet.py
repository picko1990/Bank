        ...
        if form.is_valid():
            ...
            with transaction.atomic():
                ...
                faucet_model = FaucetModel.objects.filter(
                    Q(account=account) | Q(user_id=user_id),
                    next_valid_access_time__gt=datetime.utcnow()
                ).first()
                if not faucet_model:
                    faucet_model, created = FaucetModel.update_or_create(
                        social_types = platform,
                        account = account,
                        social_user_id = user_id,
                        next_valid_access_time = datetime.utcnow()+timedelta(hours=amount.delay)
                    )
